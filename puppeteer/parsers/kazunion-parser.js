...
export class KazunionParser extends BaseParser {
  constructor() {
    super('Kazunion');
  }
  ...
  async fillFilters(filters) {
    try {
      if (filters.departure_city) {
        const input = await this.page.$('input[name*="departure"], input[placeholder*="откуда"]');
        if (input) await this.type('input[name*="departure"]', filters.departure_city);
        await this.page.waitForTimeout(500);
      }

      if (filters.destination_country) {
        const input = await this.page.$('input[name*="destination"], input[placeholder*="куда"]');
        if (input) await this.type('input[name*="destination"]', filters.destination_country);
        await this.page.waitForTimeout(500);
      }

      if (filters.departure_from) {
        const dateInput = await this.page.$('input[type="date"]');
        if (dateInput) await this.type('input[type="date"]', filters.departure_from);
        await this.page.waitForTimeout(300);
      }

      if (filters.nights_from) {
        const nightsInput = await this.page.$('input[name*="nights"]');
        if (nightsInput) await this.type('input[name*="nights"]', filters.nights_from.toString());
        await this.page.waitForTimeout(300);
      }

      if (filters.adults) {
        const adultsInput = await this.page.$('input[name*="adult"]');
        if (adultsInput) await this.type('input[name*="adult"]', filters.adults.toString());
        await this.page.waitForTimeout(300);
      }
    } catch (error) {
      console.error('[Kazunion] fillFilters error:', error.message);
    }
  }
  ...
  async submitSearch() {
    try {
      ...
      console.warn('[Kazunion] submitSearch error:', error.message);
    }
  }
  ...
  async extractTours() {
    try {
      ...
      console.log(`[Kazunion] Extracted ${tours.length} tours`);
      return { tours, debug_table_html: debugHtml };
    } catch (error) {
      console.error('[Kazunion] extractTours error:', error.message);
      return [];
    }
  }
}
